...
::PrintSelf(std::ostream& os, Indent indent) const
{
  ...
  if( this->m_BasisVectors.IsNotNull() )
    {
    os << indent << "Basis Vector count: " << this->m_BasisVectors->Size()
      << std::endl;
    if( this->m_BasisVectors->Size() )
      {
      MatrixType& thisBasis = this->m_BasisVectors->ElementAt(0);
      os << indent << "Basis Vector dimensions: " << thisBasis.rows() << "x"
        << thisBasis.cols() << std::endl;
      }
    }
  ...
  if( this->m_VectorFieldSet.IsNotNull() )
    {
    os << indent << "Vector Field Set count: " << this->m_VectorFieldSet->Size()
      << std::endl;
    }
  ...
  if( this->m_PointSet.IsNotNull() )
    {
    os << indent << "PointSet dimensions: " << m_PointSet->GetNumberOfPoints()
